package hhxy.dn.wph.constant;

/**
 * @Author: 邓宁
 * @Date: Created in 22:25 2018/11/12
 * 数据库表名常量类
 */
public class DataBaseTableConstant {
    /**
     * @Description 用户基本信息表
     */
    public static final String USER = " tb_user ";
    /**
     * @Description 用户密码表
     */
    public static final String USER_PWD = " tb_user_pwd ";
    /**
     * @Description 用户收货地址表
     */
    public static final String USER_ADDRESS = " tb_user_address ";
    /**
     * @Description 用户搜索历史表
     */
    public static final String SEARCH_HISTORY = " tb_user_search ";
    /**
     * @Description 商品一级分类目录表
     */
    public static final String PRIMARY_CATEGORY = " tb_primary_category ";
    /**
     * @Description 商品二级分类目录表
     */
    public static final String SECONDARY_CATEGORY = " tb_secondary_category ";
    /**
     * @Description 商品基本信息表
     */
    public static final String PRODUCT = " tb_product ";
    /**
     * @Description 商品颜色表
     */
    public static final String PRODUCT_COLOR = " tb_product_color ";
    /**
     * @Description 商品尺寸表
     */
    public static final String PRODUCT_SIZE = " tb_product_size ";
    /**
     * @Description 商品图片表
     */
    public static final String PRODUCT_IMAGE = " tb_product_image ";
    /**
     * @Description 商户表
     */
    public static final String SELLER = " tb_seller ";
    /**
     * @Description 购物车表
     */
    public static final String CART = " tb_cart ";
    /**
     * @Description 订单表
     */
    public static final String ORDER  =  " tb_order ";
    /**
     * @Description 订单商品详情表
     */
    public static final String ORDER_PRODUCT = " tb_order_product ";
    /**
     * @Description 用户关注商户表
     */
    public static final String USER_COLLECT_SELLER = " tb_user_collect_seller ";
    /**
     * @Description 商品库存表
     */
    public static final String PRODUCT_NUM = " tb_product_num ";
    /**
     * @Description 商品分类目录表
     */
    public static final String CATEGORY = " tb_category ";
    /**
     * @Description 商品分类属性表
     */
    public static final String CATEGORY_ATTRIBUTE = " tb_category_attribute ";
    /**
     * @Description 商品属性表
     */
    public static final String PRODUCT_ATTRIBUTE = " tb_product_attribute ";
    /**
     * @Description 商品属性值表
     */
    public static final String PRODUCT_ATTRIBUTE_VALUE = " tb_product_attribute_value ";
    /**
     * @Description 商品-属性-属性值关系表
     */
    public static final String PRODUCT_ATTRIBUTE_RELATION = " tb_product_attribute_relation ";
    /**
     * @Description 品牌表
     */
    public static final String BRAND = " tb_brand ";
    /**
     * @Description 商户账号表
     */
    public static final String SELLER_ACCOUNT = " tb_seller_account ";
    /**
     * @Description 人员-角色表
     */
    public static final String PEOPLE_ROLE = " tb_role_people ";
    /**
     * @Description 角色表
     */
    public static final String ROLE = " tb_role ";
    /**
     * @Description 资源表
     */
    public static final String RESOURCE = " tb_resource ";
    /**
     * @Description 角色-资源表
     */
    public static final String ROLE_RESOURCE = " tb_resource_role ";

}
